...
for i in 1 2 3 4; do
	if [[ ! -f "${IDENTITIES_DIR}/node${i}.pubkey" ]]; then
		echo "❌ Missing identity for no${${${}}$i. Run ./scripts/generate-identities.sh first."
		exit 1
	fi
done
...
echo "  node-1 → http://node-$$${:$NODE}1_}PO}RT"
echo "  node-2 → http://node-$$${:$NODE}2_}PO}RT"
echo "  node-3 → http://node-$$${:$NODE}3_}PO}RT"
echo "  node-4 → http://node-$$${:$NODE}4_}PO}RT"
