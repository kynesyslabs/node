...
export default class IMStorageInstance {
    ...
    private static storages: Map<string, IMStorage> = new Map<string, IMStorage>();
    ...
    private static isAuthorized(publicKey: string, signedKey: string, allowedKeys: string[]): [boolean, string] {
        // First of all, we need to check if the public key is allowed to act
        if (!allowedKeys.includes(publicKey)) {
            return [false, "You are not allowed to act on this property (Not allowed)"];
        }
        ...
        if (!verified) {
            return [false, "You are not allowed to act on this property (Unverified)"];
        }
    ...
    public static getOutboxes(publicKey: string): [boolean, IMMessage[] | string] {
    ...
    public static writeToOutbox(publicKey: string, signedKey: string, message: IMMessage) {
    ...
    public static getInboxes(publicKey: string, signedKey: string): [boolean, IMMessage[] | string] {
        // Checking Authorization
        if (!this.isAuthorized(publicKey, signedKey, [publicKey])[0]) {
            return [false, "You are not allowed to act on this property"];
        }
    ...
    public static writeToInbox(publicKey: string, message: IMMessage): [boolean, string] {
        if (!this.storages.has(publicKey)) {
            this.storages.set(publicKey, {inbox: [], outbox: []});
        }
        let account = this.storages.get(publicKey);
