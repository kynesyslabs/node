    ...
    await new Promise<void>((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            if (!resolved) {
                ...
                reject(
                    new Error(
                        `wstcp startup timeout - no response after ${PORT_CONFIG.SPAWN_TIMEOUT_MS}ms`,
                    ),
                )
            }
        ...
        childProcess.stderr?.on("data", (data: Buffer) => {
            ...
            if (
                ...
                if (!resolved) {
                    ...
                    if (addrInUse) {
                        ...
                        reject(
                            new Error(
                                `wstcp panic: ${output.trim().substring(0, 200)}`,
                            ),
                        )
                    }
                }
            ...
            if (
                ...
                output.includes("Starts a WebSocket")
            ) {
                clearTimeout(timeout)
                if (!resolved) {
                    cleanup()
                    log.info(
                        `[TLSNotary] wstcp started successfully on port ${localPort}`,
                    )
                    resolve()
                }
                return
        ...
        childProcess.on("exit", code => {
            ...
            if (!resolved) {
                ...
                if (code !== null && code !== 0) {
                    reject(
                        new Error(
                            `wstcp exited with code ${code}: ${stderrBuffer.trim().substring(0, 200)}`,
                        ),
                    )
                }
            }
...
export async function requestProxy(
    ...
    if (existingProxy && isProxyAlive(existingProxy)) {
        ...
        log.info(
            `[TLSNotary] Reusing existing proxy for ${domain}:${targetPort}`,
        )
        return {
...
export function killProxy(proxyId: string): boolean {
    ...
    for (const [key, proxy] of state.proxies) {
        if (proxy.proxyId === proxyId) {
            log.info(
                `[TLSNotary] Manually killing proxy ${proxyId} for ${proxy.domain}`,
            )
            try {
                proxy.process.kill("SIGTERM")
            } catch {
