...
function resolveSigningKey(): string | null {
    ...
    if (envKey && envKey.length === 64) {
        ...
        log.warning(
            "[TLSNotary] TLSNOTARY_SIGNING_KEY must be 64 hex characters (32 bytes)",
        )
        return null
    }
    ...
    if (existsSync(keyFilePath)) {
        try {
            ...
            if (fileKey.length === 64) {
                log.info(
                    `[TLSNotary] Using signing key from ${SIGNING_KEY_FILE}`,
                )
                return fileKey
            } else {
                log.warning(
                    `[TLSNotary] Invalid key in ${SIGNING_KEY_FILE} (must be 64 hex characters)`,
                )
                return null
            }
        } catch (error) {
            log.warning(
                `[TLSNotary] Failed to read ${SIGNING_KEY_FILE}: ${error}`,
            )
            return null
        }
    }
    ...
    try {
        ...
        log.info(
            `[TLSNotary] Auto-generated signing key saved to ${SIGNING_KEY_FILE}`,
        )
        return generatedKey
    } catch (error) {
...
export function getConfigFromEnv(): TLSNotaryServiceConfig | null {
    ...
    const mode = (
        process.env.TLSNOTARY_MODE?.toLowerCase() === "ffi" ? "ffi" : "docker"
    ) as TLSNotaryMode
    ...
    if (mode === "ffi") {
        ...
        if (!signingKey) {
            log.warning(
                "[TLSNotary] Failed to resolve signing key for FFI mode",
            )
            return null
        }
    }
    ...
    return {
        ...
        maxSentData: parseInt(
            process.env.TLSNOTARY_MAX_SENT_DATA ?? "16384",
            10,
        ),
        maxRecvData: parseInt(
            process.env.TLSNOTARY_MAX_RECV_DATA ?? "65536",
            10,
        ),
...
export class TLSNotaryService {
    ...
    async initialize(): Promise<void> {
        ...
        if (fatal) {
            log.warning(
                "[TLSNotary] Fatal mode enabled - errors will cause process exit",
            )
        }
    ...
    private async initializeDockerMode(): Promise<void> {
        ...
        if (debug) {
            log.info(
                `[TLSNotary] Docker mode: expecting container on port ${this.config.port}`,
            )
        }
        ...
        log.info(
            "[TLSNotary] Docker mode initialized (container managed externally)",
        )

        if (debug) {
            log.info(`[TLSNotary] Config: port=${this.config.port}`)
            log.info(
                "[TLSNotary] Container should be started via: cd tlsnotary && docker compose up -d",
            )
        }
    }
    ...
    private async initializeFFIMode(): Promise<void> {
        ...
        try {
            ...
            if (debug) {
                log.info(
                    `[TLSNotary] Config: port=${this.config.port}, maxSentData=${this.config.maxSentData}, maxRecvData=${this.config.maxRecvData}`,
                )
            }
        } catch (error) {
            ...
            if (fatal) {
                log.error(
                    "[TLSNotary] FATAL: Exiting due to initialization failure",
                )
                process.exit(1)
            }
            throw error
    ...
    private async startDockerMode(): Promise<void> {
        ...
        try {
            ...
            if (this.dockerPublicKey) {
                log.info(
                    `[TLSNotary] Notary public key: ${this.dockerPublicKey}`,
                )
            }
        } catch (error) {
            const message =
            ...
            if (fatal) {
                log.error(
                    "[TLSNotary] FATAL: Exiting due to Docker container not available",
                )
                process.exit(1)
            }
            throw new Error(
                `Docker notary container not accessible: ${message}`,
            )
        }
    }
    ...
    private async startFFIMode(): Promise<void> {
        ...
        try {
            if (debug) {
                log.info(
                    `[TLSNotary] Starting WebSocket server on port ${this.config.port}...`,
                )
                log.info(
                    "[TLSNotary] NOTE: TLSNotary only accepts WebSocket connections via HTTP GET",
                )
                log.info(
                    "[TLSNotary] Non-GET requests (POST, PUT, etc.) will fail with WebSocket upgrade error",
                )
            }
            ...
            this.running = true
            log.info(
                `[TLSNotary] FFI server started on port ${this.config.port}`,
            )

            if (debug) {
                log.info(
                    `[TLSNotary] Public key: ${this.ffi.getPublicKeyHex()}`,
                )
            ...
            if (proxyEnabled) {
                log.warning(
                    "[TLSNotary] DEBUG PROXY ENABLED - All incoming data will be logged!",
                )
            }
        } catch (error) {
            log.error(
                `[TLSNotary] Failed to start FFI server on port ${this.config.port}: ${error}`,
            )
            if (fatal) {
                log.error(
                    "[TLSNotary] FATAL: Exiting due to server start failure",
                )
                process.exit(1)
            }
            throw error
    ...
    private async startWithProxy(): Promise<void> {
        ...
        this.proxyServer = net.createServer(clientSocket => {
            ...
            const rustSocket = net.connect(rustPort, "127.0.0.1", () => {
                log.debug(
                    `[TLSNotary-Proxy] Connected to Rust server for ${clientAddr}`,
                )
            })
            ...
            clientSocket.on("data", data => {
                ...
                log.info(
                    `[TLSNotary-Proxy] <<< FROM ${clientAddr} (${data.length} bytes):`,
                )
            ...
            clientSocket.on("error", err => {
                log.warning(
                    `[TLSNotary-Proxy] Client error ${clientAddr}: ${err.message}`,
                )
                rustSocket.destroy()
            })
            ...
            rustSocket.on("error", err => {
                log.warning(
                    `[TLSNotary-Proxy] Rust connection error for ${clientAddr}: ${err.message}`,
                )
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            this.proxyServer!.listen(publicPort, () => {
                log.info(
                    `[TLSNotary-Proxy] Listening on port ${publicPort}, forwarding to ${rustPort}`,
                )
                resolve()
            })
        })
    ...
    async stop(): Promise<void> {
        ...
        if (mode === "docker") {
            ...
            this.running = false
            log.info(
                "[TLSNotary] Docker mode - marked as stopped (container still running)",
            )
            return
        }
    ...
    async shutdown(): Promise<void> {
        ...
        if (mode === "docker") {
            ...
            log.info(
                "[TLSNotary] Docker mode - service shutdown complete (container still running)",
            )
            return
        }
    ...
    getPublicKey(): Uint8Array {
        ...
        if (mode === "docker") {
            if (!this.dockerPublicKey) {
                throw new Error(
                    "Docker public key not available - service not started",
                )
            }
