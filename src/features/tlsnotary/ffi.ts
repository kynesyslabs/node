...
export class TLSNotaryFFI {
    ...
    constructor(config: NotaryConfig) {
        ...
        if (initResult !== 0) {
            throw new Error(
                `Failed to initialize TLSNotary library: error code ${initResult}`,
            )
        }
    ...
    private createNotary(): void {
        ...
        configView.setBigUint64(
            16,
            BigInt(this.config.maxSentData ?? 16384),
            true,
        ) // max_sent_data
        configView.setBigUint64(
            24,
            BigInt(this.config.maxRecvData ?? 65536),
            true,
        ) // max_recv_data
    ...
    async startServer(port = 7047): Promise<void> {
        ...
        const result = this.lib.symbols.tlsn_notary_start_server(
            this.handle as any,
            port,
        )
    ...
    verifyAttestation(attestation: Uint8Array): VerificationResult {
        ...
        const resultPtr = this.lib.symbols.tlsn_verify_attestation(
            attestationPtr,
            BigInt(attestation.length),
        )
        ...
        try {
            ...
            if (errorMessagePtr !== 0n) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                errorMessage = new CString(
                    Number(errorMessagePtr) as any,
                ).toString()
            }
