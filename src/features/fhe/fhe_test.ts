...
async function main() {
    ...
    const cipheredAdditionResult = await fhe.math.addNumbers(
        cipheredData,
        cipheredAddStep,
    )
    // Decrypt the CipherText
    const decryptedAdditionResult = await fhe.encryption.decryptNumber(
        cipheredAdditionResult,
    )
    log.info(
        "plainData: ",
        plainData,
    ...
    log.info(
        "\n[OK] Now the cipheredData is equal to decryptedAdditionResult: ",
        decryptedData,
    )
    log.info("\n[Multiplication]")
    const cipheredMultiplyStep =
        await fhe.encryption.encryptNumber(multiplyStep)
    // Multiply the CipherText to itself and store it in the destination parameter (itself)
    const cipheredMultiplicationResult = await fhe.math.multiplyNumbers(
        cipheredData,
        cipheredMultiplyStep,
    )
    // Decrypt the CipherText
    const decryptedMultiplicationResult = await fhe.encryption.decryptNumber(
        cipheredMultiplicationResult,
    )
    log.info(
        "plainData: ",
        plainData,
        "\nmultiplyStep: ",
        multiplyStep,
        "\ndecryptedMultiplyResult: ",
        decryptedMultiplicationResult,
    )
    ...
    log.info(
        "\n[OK] Now the cipheredData is equal to decryptedMultiplicationResult: ",
        decryptedData,
    )
    ...
    // Decrypt the CipherText
    const decryptedNegateResult =
        await fhe.encryption.decryptNumber(cipheredNegateResult)
    ...
    log.info(
        "\n[OK] Now the cipheredData is equal to -decryptedNegateResult: ",
        decryptedData,
    )
}
