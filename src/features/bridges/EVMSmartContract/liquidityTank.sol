...
contract LiquidityTank {
    ...
    function setAuthorizedAddresses(
        ...
        if (_addresses.length < 1) revert InsufficientAddresses();
        if (_addresses.length > MAX_AUTHORIZED_ADDRESSES)
    ...
    function emergencyResetOwners(
        address[] memory _addresses
    ) external onlyDeployer {
        if (!initialized) revert NotInitialized();
        if (block.timestamp < lastOwnershipRotation + EMERGENCY_TIMEOUT)
    ...
    function multisigTransfer(
        ...
        if (proposal.deadline == 0) {
            ...
            if (keccak256(proposal.data) != keccak256(data))
                revert ProposalDataMismatch();
        }
    ...
    function proposeNextOwners(
        ...
        if (newOwners.length > MAX_AUTHORIZED_ADDRESSES)
            revert TooManyAddresses();
    ...
    function _executeTransfer(bytes memory data) internal {
        ...
        if (token == address(0)) {
            ...
            actualAmount = _safeERC20TransferWithSlippage(
                token,
                to,
                amount,
                slippageBps
    ...
    function _executeOwnershipRotation(
        /*bytes32 proposalId, */ bytes memory data
    ) internal {
        (string memory action, address[] memory newOwners) = abi.decode(
            data,
            (string, address[])
        );

        if (keccak256(bytes(action)) != keccak256("ROTATE_OWNERS"))
            revert InvalidAction();
    ...
    function _executeEmergencyWithdraw(
        ...
        (string memory action, address token, address to, uint256 amount) = abi
            .decode(data, (string, address, address, uint256));

        if (keccak256(bytes(action)) != keccak256("EMERGENCY"))
            revert InvalidAction();
    ...
    function _payGasFromPool(address gasPayee) internal {
        ...
        if (gasSubsidyPool < gasReimbursement)
            gasReimbursement = gasSubsidyPool;
    ...
    function _createBridgeMessageHash(
    ...
    function depositUSDCToTank(
        ...
        if (actualDeposited < (amount * 9900) / 10000)
    ...
    function initiateBridgeOperation(
        ...
        if (
            (
                params.token == address(0)
                    ...
                    : _getERC20Balance(params.token, address(this))
            ) < params.amount
        ) {
        ...
        uint256 amountAfterFee = (params.amount *
    ...
    function depositAndBridge(
        ...
        if (
            ...
            keccak256(bytes(tokenName)) != keccak256("eth")
        ) {
        ...
        if (tokenAddress == address(0)) {
            ...
            uint256 balanceBefore = _getERC20Balance(
                tokenAddress,
                address(this)
            );
            ...
            uint256 balanceAfter = _getERC20Balance(
                tokenAddress,
                address(this)
            );
            uint256 actualDeposited = balanceAfter - balanceBefore;
        ...
        // Calculate amount after fee
        uint256 amountAfterFee = (depositAmount * (10000 - bridgeFeeBps)) /
            10000;
        if (amountAfterFee == 0) revert InvalidAmount();
    ...
    function _safeERC20TransferWithSlippage(
        address token,
        address to,
        uint256 amount,
        uint256 slippageBps
        ...
        uint256 contractBalanceChange = contractInitialBalance -
