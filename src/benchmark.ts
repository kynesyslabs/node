    ...
    try {
        ...
        for (const [component, details] of Object.entries(result.details)) {
            ...
            if (component === "network") {
                const networkValue = details.value as {
                    download: number
                    upload: number
                }
                console.log(
                    `    Download Speed: ${networkValue.download.toFixed(2)} Mbps`,
                )
                console.log(
                    `    Upload Speed: ${networkValue.upload.toFixed(2)} Mbps`,
                )
            } else {
                console.log(
                    `    Detected Value: ${(details.value as number).toFixed(2)} ${getUnit(component)}`,
                )
            }
        }
        ...
        if (!result.meetsMinimum) {
            ...
            console.log(
                "Check the .requirements file for minimum specifications.",
            )
            process.exit(1)
            ...
            console.log(
                "Consider upgrading to suggested specifications for optimal performance.",
            )
            process.exit(0)
        } else {
