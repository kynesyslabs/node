/*
    ...
    are part of.
*/
import IPartecipant from "./Partecipants"
...
export default class L2PS {
    ...
    constructor(create: boolean, _rsaKeyPair?: forge.pki.rsa.KeyPair, _uid?: string) {
        ...
        if (!create) {
            ...
            let existing = this.fetchFromGLS(Hashing.sha256(_rsaKeyPair.publicKey.n.toString(16)))
    ...
    public encryptForL2PS(message: string): string {
        // TODO Encrypt the message for the L2PS
        return ""
    }

    public decryptFromL2PS(message: string): string {
        // TODO Decrypt the message from the L2PS
        return ""
    }
    ...
    public async fetchFromGLS(uid: string): Promise<void> {
    ...
    // Check if a partecipant is part of the L2PS
    public isPartecipant(partecipant: IPartecipant): boolean {
        return this.partecipants.find(p => p.id === partecipant.id) !== undefined
    }
