...
export default async function applyGCROperation(operation: GCROperation): Promise<boolean> {
    // Get the GCR tables
    let success = true
    const db = await Datasource.getInstance()
    const gcrTrackerRepository: Repository<GCRTracker> = db.getDataSource().getRepository(GCRTracker)
    const gcrHashesRepository: Repository<GCRHashes> = db.getDataSource().getRepository(GCRHashes)
    const gcrSubnetsTxsRepository: Repository<GCRSubnetsTxs> = db.getDataSource().getRepository(GCRSubnetsTxs)
    const globalChangeRegistryRepository: Repository<GlobalChangeRegistry> = db.getDataSource().getRepository(GlobalChangeRegistry)
