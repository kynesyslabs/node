...
export class SolanaDomainResolver {
    ...
    private deriveSldPda(
        ...
        const [result] = PublicKey.findProgramAddressSync(
            [
                version,
                Buffer.from("sld"),
                this.hashSeedStr(tld),
                this.hashSeedStr(label),
            ],
            this.unsProgramId,
        )
        return result
     ...
     */
    private deriveDomainPropertiesPda(
        sldPda: PublicKey,
        version = this.defaultVersion,
    ): PublicKey {
    ...
    private async getTokenOwner(
        ...
        try {
            ...
            const tokenAccounts =
                await connection.getTokenLargestAccounts(sldPda)

            if (tokenAccounts.value.length === 0) {
                log.debug(
                    `No token accounts found for mint ${sldPda.toString()}`,
                )
                return undefined
            }
            ...
            if (
                tokenAccountInfo.value &&
                "data" in tokenAccountInfo.value.data
            ) {
            ...
            log.debug(
                `Could not extract owner from token account ${tokenAccounts.value[0].address.toString()}`,
            )
            return undefined
        } catch (error) {
            log.debug(
    ...
    async resolveRecord(
        ...
        try {
            ...
            try {
                domainProperties =
                    await program.account.domainProperties.fetch(
                        domainPropertiesPda,
                    )
            } catch (error) {
                return {
    ...
    async getDomainInfo(
        label: string,
        tld: string,
    ): Promise<Omit<DomainResolutionResult, "records">> {
