...
export default async function manageProposeBlockHash(
    ...
    if (ourCandidateHash === blockHash) {
        const incomingValidationData = validationData["signatures"]

        for (const identity in incomingValidationData) {
            const signature = incomingValidationData[identity]
            ...
            if (isSignatureValid) {
                log.debug(
                    `[manageProposeBlockHash] Incoming signature is valid for peer: ${identity}. Adding it to our candidate block validation data`,
                )
