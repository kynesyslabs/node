...
export default class ServerHandlers {
    ...
    static async handleValidateTransaction(
        ...
        try {
            ...
            // REVIEW: Token edits are not yet generated by SDK's GCRGeneration (Phase 1.4),
            // so we only compare the non-token edits for now.
            const txEdits = Array.isArray(tx.content.gcr_edits)
                ? (tx.content.gcr_edits as any[])
                : []
            const txNonTokenEdits = txEdits.filter((e) => e?.type !== "token")
            const tokenEditsCount = txEdits.length - txNonTokenEdits.length

            const txGcrEditsHash = Hashing.sha256(
                JSON.stringify(txNonTokenEdits),
            )
            log.debug(
