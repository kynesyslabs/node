...
export default class ComLinkUtils {
    ...
    static createComLink(
        ...
        new_comlink.chain.current.currentMessageHash = Hashing.sha256(JSON.stringify(new_comlink.chain.current.currentMessage))
        new_comlink.chain.current.previousHashes = previousHashes
        new_comlink.chain.comlinkCurrentHash = Hashing.sha256(JSON.stringify(new_comlink.chain.current))
