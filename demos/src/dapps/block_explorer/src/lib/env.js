...
export const updateWallet = () => {
  ...
  if (prvkey) { localStorage.setItem('prvkey', demos.DemosWebAuth.getInstance().stringified_keypair.privateKey) } else { localStorage.removeItem('prvkey') }
}
...
export const trim_address = (str, length) => {
  if (str.length <= length - 3 || str.length <= 20) { return str }
...
export const normalize_timestamp = (timestamp) => {
  if (timestamp.length == 10) { return blocks[i].content.timestamp = blocks[i].content.timestamp * 1000 }
  return timestamp
}
