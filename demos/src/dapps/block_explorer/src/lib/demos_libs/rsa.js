...
export default class RSA {
	...
	refresh() {
		if (DemosWebAuth.getInstance().stringified_keypair) {
			let pre_seed = DemosWebAuth.getInstance().stringified_keypair.privateKey
		...
		let seed = this.seedForger(pre_seed)
		let pseudorandom = this.PRNG(seed)
	...
	self_encrypt(message) {
		...
		let encoded = forge.util.encode64(message)
        let encrypted = this.keypair.publicKey.encrypt(message)
    ...
    decrypt(message) {
		if (!required(this.keypair)) throw new Error("RSA Keypair cannot be generated without a proper ECDSA keypair")
		let debased_encrypted = forge.util.decode64(message)
        let decrypted = this.keypair.privateKey.decrypt(debased_encrypted)
		return decrypted
    }
}
