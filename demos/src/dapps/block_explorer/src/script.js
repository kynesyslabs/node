...
async function watchdog() { // Executed every 2 seconds
    ...
    while (true) {
        ...
        if (demos.connected) {
            ...
            // NOTE Adding this to the GUI as an example
            latestBlocks.innerHTML = "<p class=\"lineblock\"> " + JSON.parse(last_number).number + " - " + 
                                    "<a href=\"javascript:inspectBlock('" + JSON.parse(last_number).number + "')\">" + 
                                    JSON.parse(last_hash).hash + "</a>" + "</p>" 
        }
    }
}
