...
export class EnigmaSession {
    ...
    constructor(name: string) {
		...
		console.log("Warning: initialize with 'await this.init()' before using this class")
    }
	...
	async newMessage(message: any): Promise<any> {
		this.shortTermKey = await this.enigma.generateSecrets(this.longTermKeys.their)
		let encrypted = await this.enigma.encrypt(message, Buffer.from(this.shortTermKey.privateKey).toString("hex"))
		return encrypted
	}
	...
	async readMessage(message: any, encapsulation: any): Promise<any> {
		...
		let decrypted = await this.enigma.decrypt(message, Buffer.from(this.shortTermKey.privateKey).toString("hex"))
		return decrypted
	}
