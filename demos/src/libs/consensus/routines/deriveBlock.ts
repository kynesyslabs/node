...
export default async function deriveBlock(mempoolData: MempoolData): Promise<Block> {
	...
	let proposer = sharedState.getInstance().identity.ed25519.publicKey.toString("hex")
	derivedBlock.proposer = proposer
	derivedBlock.timestamp = new Date().getTime()
	// REVIEW Order transactions
	let ordered_transactions: Transaction[] = await orderTxs(mempoolData.transactions)
