...
var listeners = {
    ...
    server_listeners: async function () {
        ...
        io_server
            .on("connection", async peerSocket => {
                term.magenta("[SERVER] Pcaer connection incoming\n")
                ...
                peerSocket.on("comlink", async request => {
                    ...
                    if (content.type === "nodeCall") {
                        ...
                        _comlink_request.properties.require_reply =
                            require_reply // Setting the require_reply flag as provided above
                ...
                peerSocket.on("transactions", async request => {
                    ...
                    let verification = await transactions.methods.verify(
                        request.tx,
                    )
                    if (!verification[0]) {
                        peerSocket.emit("transactions", {
