...
describe("Consensus Fixtures - proposeBlockHash", () => {
    ...
    fixtures.forEach(fixtureFile => {
        it(`should decode and encode ${fixtureFile} correctly`, () => {
            ...
            const [blockHash, validationData, proposer] = consensusPayload.params as [
                string,
            ...
            const { PrimitiveEncoder } = require("@/libs/omniprotocol/serialization/primitives")
            ...
            const normalizeHex = (hex: string) => hex.toLowerCase().replace(/^0x/, "")
            expect(normalizeHex(decoded.blockHash)).toBe(normalizeHex(blockHash))
            ...
            const responsePayload = {
                ...
                signatures: (fixture.response.extra as { signatures: Record<string, string> })
                    ?.signatures ?? {},
            }

            const encodedResponse = encodeProposeBlockHashResponse(responsePayload)
...
describe("Consensus Fixtures - setValidatorPhase", () => {
    ...
    fixtures.forEach(fixtureFile => {
        it(`should decode and encode ${fixtureFile} correctly`, () => {
            ...
            const [phase, seed, blockRef] = consensusPayload.params as [number, string, number]
            ...
            const responsePayload = {
                status: fixture.response.result,
                greenlight: (fixture.response.extra as { greenlight: boolean })?.greenlight ?? false,
                timestamp: BigInt(
                    (fixture.response.extra as { timestamp: number })?.timestamp ?? 0,
                ),
                blockRef: BigInt((fixture.response.extra as { blockRef: number })?.blockRef ?? 0),
            }

            const encodedResponse = encodeSetValidatorPhaseResponse(responsePayload)
            expect(encodedResponse).toBeInstanceOf(Buffer)
            expect(encodedResponse.length).toBeGreaterThan(0)
        })
    })
...
describe("Consensus Round-Trip Encoding", () => {
    it("proposeBlockHash should encode and decode without data loss", () => {
        const original: ProposeBlockHashRequestPayload = {
            blockHash: "0xabc123def456789012345678901234567890123456789012345678901234abcd",
            validationData: {
                "0x1111111111111111111111111111111111111111111111111111111111111111": "0xaaaa",
                "0x2222222222222222222222222222222222222222222222222222222222222222": "0xbbbb",
            },
            proposer: "0x3333333333333333333333333333333333333333333333333333333333333333",
        }
        ...
        const normalizeHex = (hex: string) => hex.toLowerCase().replace(/^0x/, "")
        expect(normalizeHex(decoded.blockHash)).toBe(normalizeHex(original.blockHash))
        expect(normalizeHex(decoded.proposer)).toBe(normalizeHex(original.proposer))
        expect(Object.keys(decoded.validationData).length).toBe(
