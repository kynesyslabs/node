...
| Multichain / XM parser and dispatcher | Yes | `multichain_parser_rejects`, `multichain_parser_execute_smoke`, `multichain_dispatcher_aggregation` | `better_testing/runs/_latest/prod-gate.latest.md` and local multichain artifacts from 2026-03-12 | `multichain_parser_rejects` is gated | Reject behavior is release-gated; execute happy path exists but is not yet in the gate |
