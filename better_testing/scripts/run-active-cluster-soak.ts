...
async function main() {
  ...
  const clusterCmd = [
    "bun",
    "better_testing/scripts/run-suite.ts",
    "cluster-health",
    "--local",
  ...
  const summary = {
    ...
    runTag,
    command: `bun better_testing/scripts/run-active-cluster-soak.ts${args.build ? " --build" : ""}${args.quiet ? " --quiet" : " --verbose"}`,
    steps: {
      preCheckCopyPath,
      transferSummaryPath,
      zkSummaryPath,
  ...
  console.log("Latest report:     better_testing/runs/_latest/cluster-soak.latest.md")
}
