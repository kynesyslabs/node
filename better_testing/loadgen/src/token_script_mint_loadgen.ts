...
import { sleep, envInt, envBool, splitCsv, nowMs, unique, percentile, ReservoirSampler } from "./testing_utils"
...
function getConfig(): LoadgenConfig {
  ...
  return {
    ...
    amount: BigInt(process.env.TOKEN_MINT_AMOUNT ?? process.env.AMOUNT ?? "1"),
...
export async function runTokenScriptMintLoadgen() {
  ...
  const artifactBase = `${run.runDir}/token_script_mint`
  const artifacts = {
    runId: run.runId,
    runDir: run.runDir,
  ...
  const settleSample = unique([senderHex, ...recipients]).slice(0, envInt("POST_RUN_SETTLE_SAMPLE_ADDRESSES", 8))
  ...
  const summary = {
    ...
    config: {
      ...
      avoidSelfRecipient: cfg.avoidSelfRecipient,
    },
    postRun: {
      settleSample,
  ...
  console.log(JSON.stringify({ token_script_mint_summary: summary }, null, 2))

  const strict = envBool("POST_RUN_SETTLE_STRICT", true)
  if (strict && postRunSettleCheck && postRunSettle && !postRunSettle.ok) {
    throw new Error("Post-run settle check failed (token_script_mint)")
  }
