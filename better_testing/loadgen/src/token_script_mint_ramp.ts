...
export async function runTokenScriptMintRamp() {
  ...
  const artifactBase = `${run.runDir}/token_script_mint_ramp`
  const artifacts = {
    runId: run.runId,
    runDir: run.runDir,
  ...
  for (const step of steps) {
    process.env.SCENARIO = "token_script_mint"
      ...
      for (const row of capture) {
        ...
        if (payload && typeof payload === "string") {
          try {
            ...
            if (parsed?.token_script_mint_summary) stepSummary = parsed.token_script_mint_summary
          } catch (error) {
            logNonCriticalError("token_script_mint_ramp.captureSummary", error, step)
          }
        } else if (payload?.token_script_mint_summary) {
  ...
  const rampSummary = {
    scenario: "token_script_mint_ramp",
    mode,
    bestByOkTps: best,
    steps: results,
    config: {
  ...
  console.log(JSON.stringify({ token_script_mint_ramp_summary: rampSummary }, null, 2))
}
