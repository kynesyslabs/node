...
function usage() {
  console.log(`Usage:
  bun testing/scripts/run-active-core-baseline.ts [--build] [--quiet|--verbose] [--with-omni]
...
function loadDevnetRecipients(): string {
  ...
  if (recipients.length === 0) {
    throw new Error("Could not load devnet recipient pubkeys from testing/devnet/identities")
  }
...
async function main() {
  ...
  const runDir = path.join(process.cwd(), "testing", "runs", `baseline-active-core-${runTag}`)
  ensureDir(runDir)
  ...
  const configs: StepConfig[] = [
    {
      ...
      scenario: "transfer",
      summaryPath: path.join(process.cwd(), "testing", "runs", "{runId}", "transfer.summary.json"),
      env: {
    ...
    },
    {
      ...
      summaryPath: path.join(process.cwd(), "testing", "runs", "{runId}", "features", "zk", "zk_proof_loadgen.summary.json"),
      env: {
        DURATION_SEC: "10",
        CONCURRENCY: "2",
      },
    },
    {
      ...
      summaryPath: path.join(process.cwd(), "testing", "runs", "{runId}", "features", "peersync", "sync_under_load.summary.json"),
      env: {
        DURATION_SEC: "15",
        CONCURRENCY: "4",
      },
  ...
  if (args.withOmni) {
    configs.push({
      ...
      summaryPath: path.join(process.cwd(), "testing", "runs", "{runId}", "features", "omni", "omni_throughput.summary.json"),
      env: {
        DURATION_SEC: "10",
        CONCURRENCY: "2",
      },
  ...
  const summary = {
    ...
    runTag,
    command: `bun testing/scripts/run-active-core-baseline.ts${args.build ? " --build" : ""}${args.withOmni ? " --with-omni" : ""}${args.quiet ? " --quiet" : " --verbose"}`,
    results,
  ...
  const latestDir = path.join(process.cwd(), "testing", "runs", "_latest")
  ...
  console.log("Latest report:     testing/runs/_latest/active-core-baseline.latest.md")
}
