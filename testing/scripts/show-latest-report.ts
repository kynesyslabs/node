...
function usage(): never {
  console.log(`Usage:
  bun testing/scripts/show-latest-report.ts
  bun testing/scripts/show-latest-report.ts <suite>

Examples:
  bun testing/scripts/show-latest-report.ts
  bun testing/scripts/show-latest-report.ts sanity
`)
  process.exit(0)
}
...
function main() {
  ...
  if (reportPaths.length === 0) {
    console.log("No latest suite reports found in testing/runs/_latest")
    process.exit(1)
  }
  ...
  if (!fs.existsSync(reportPath)) {
    console.log(`No latest report found for suite '${arg}' in testing/runs/_latest`)
    process.exit(1)
  }
