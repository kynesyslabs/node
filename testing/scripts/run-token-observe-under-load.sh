...
run_one() {
  ...
  if [[ "$mode" == "scripted" ]]; then
    "$ROOT_DIR/testing/scripts/run-scenario.sh" "$bootstrap_scenario" --run-id "$RUN_ID" --quiet \
      --env DURATION_SEC=3 \
      --env CONCURRENCY=1 \
  ...
  local bootstrap_summary="$ROOT_DIR/testing/runs/$RUN_ID/${bootstrap_summary_name}.summary.json"
  ...
  local ts_path="$ROOT_DIR/testing/runs/$RUN_ID/token_observe.timeseries.jsonl"
  if [[ ! -f "$ts_path" ]]; then
  ...
  bun "$ROOT_DIR/testing/scripts/analyze-token-observe.ts" "$ts_path" --tail "$TAIL"
