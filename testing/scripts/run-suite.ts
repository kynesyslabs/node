...
function usage() {
  console.log(`Usage:
  bun testing/scripts/run-suite.ts [suite] [--build-first] [--verbose] [--local]
  bun testing/scripts/run-suite.ts --scenarios a,b,c [--build-first] [--verbose] [--local]
...
function composeCmd(args: string[], verbose: boolean): void {
  const proc = Bun.spawnSync({
    ...
    cwd: path.join(process.cwd(), "testing", "devnet"),
...
function getSuiteRunDir(suite: string, runTag: string): string {
  return path.join(process.cwd(), "testing", "runs", `suite-${suite}-${runTag}`)
}
...
function renderSuiteMarkdown(
  ...
  lines.push(`- Timestamped report: \`testing/runs/_latest/${args.suite}-${runTag}.md\``)
  lines.push(`- Latest report pointer: \`testing/runs/_latest/${args.suite}.latest.md\``)
  lines.push("")
  return `${lines.join("\n").trim()}\n`
}
...
async function runScenario(
  ...
  const runDir = path.join(process.cwd(), "testing", "runs", runId)
