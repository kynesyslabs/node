...
export async function runL2psLiveSubmissionRelaySmoke() {
  ...
  const mnemonicFile = process.env.L2PS_LIVE_MNEMONIC_FILE ?? "testing/devnet/identities/node1.identity"
  const keyPath = process.env.L2PS_LIVE_KEY_PATH ?? "testing/devnet/l2ps/live_local_001/private_key.txt"
  const ivPath = process.env.L2PS_LIVE_IV_PATH ?? "testing/devnet/l2ps/live_local_001/iv.txt"
